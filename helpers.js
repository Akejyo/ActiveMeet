...
export function sanitizeRequestBody(req, res, next) {
  if (req.body && typeof req.body === 'object') {
    for (let key in req.body) {
      // password shouldn't be rendered back to the page except current bad prev behavior
      if (
...
export function checkFirstName(name) {
  ...
  if (name.length < 3 || name.length > 20)
    throw 'First name has to be between 3 and 20 characters'
  let accept1 = /^[a-zA-Z]+$/
  if (!accept1.test(name))
    throw 'Invalid character in first name, can only contain letters'
  return name
}
...
export function checkLastName(name) {
  ...
  name = name.trim()
  if (name.length < 3 || name.length > 20)
    throw 'Last name has to be between 3 and 20 characters'
  let accept1 = /^[a-zA-Z\s]+$/
  if (!accept1.test(name))
    throw 'Invalid character in last name, can only contain letters and spaces'
  return name
}
...
export async function checkEmail(email) {
  ...
  if (
    par[0][0] === '.' ||
    par[0][par[0].length - 1] === '.' ||
    par[1][0] === '.' ||
    par[1][par[1].length - 1] === '.'
  )
...
export function checkCity(city) {
  ...
  if (city.length < 1 || city.length > 45)
    throw 'Length of city needs to be between 1 and 45' //shortest city name in US is 1 char and largest is 45 chars
  let accept1 = /^[a-zA-Z\s]+$/
  if (!accept1.test(city))
    throw 'Invalid character in city, can only contain letters'
  return city
}
...
export function checkSkillLevel(sk) {
  ...
  sk = sk.toLowerCase()
  if (!skillLevels.includes(sk))
    throw 'Invalid skill level provided (beginner, intermediate, advanced)'
  return sk
}
...
export function checkVisibility(vis) {
  ...
  if (vis != 'public' && vis != 'private')
    throw 'Visibility has to be either public or private'
  return vis
}
...
export function checkTitle(title) {
  ...
  if (title.length < 3 || title.length > 50)
    throw 'Title has to be between 3 and 50 characters'
...
export function checkSearchText(searchText) {
  ...
  if (searchText.length > 100)
    throw 'Search text has to be between 0 and 100 characters'
  return searchText
}
...
export function checkDescription(desc) {
  ...
  desc = desc.trim()
  if (desc.length < 5)
    throw 'Please write something for description, (over 5 chars)'
...
export function checkDateAndTime(dateAndTime) {
  ...
  if (!(dateAndTime instanceof Date))
    throw 'Date and time has to be a date object'
...
export function parsAndCheckAgeRestriction(str) {
  //format 12-120, 18-50  to {min: 12, max: 120}
  if (!str) throw 'No age restriction string provided'
  if (typeof str != 'string') throw 'Age restriction has to be a string'
  str = str.trim()
  let parts = str.split('-')
  if (parts.length != 2)
    throw "Invalid age restriction format. Please use the format 'min-max'"
...
export function checkAgeRestrictions(obj) {
  ...
  if (Array.isArray(obj))
    throw 'Age restrictions has to be an object, not an array'
  if (!obj.min || !obj.max)
    throw 'Age restrictions has to have min and max properties'
  if (Object.keys(obj).length != 2)
    throw 'Age restrictions can only have min and max properties'
  if (typeof obj.min != 'number' || typeof obj.max != 'number')
    throw 'Age restrictions has to be a number'
...
export function checkSkillLevelRestriction(sk) {
  ...
  sk = sk.toLowerCase()
  if (!skillLevels.includes(sk))
    throw 'Invalid skill level restriction provided (beginner, intermediate, advanced, all levels)'
  return sk
}
...
export function checkGenderRestriction(gr) {
  ...
  gr = gr.toLowerCase()
  if (!genders.includes(gr))
    throw 'Invalid gender restriction provided (male only, female only, co-ed)'
  return gr
}

export function checkLocation(locObj) {
  //Works for string now, could change to be object
...
export function checkComment(comment) {
  ...
  if (comment.length < 2 || comment.length > 300)
    throw 'Comment has to be between 2 and 300 characters'
...
export function checkComments(cmm) {
  ...
  for (let i = 0; i < cmm.length; i++) {
    if (typeof cmm[i] !== 'object' || Array.isArray(cmm[i]))
      throw 'Each comment has to be an object' //{user: 'name', content; 'string comment'
    if (!cmm[i].user || !cmm[i].content)
      throw 'Each comment has to have user and content'
    if (Object.keys(cmm[i]).length != 2)
      throw 'Each comment can only have user and content fields'
    if (typeof cmm[i].user != 'string' || typeof cmm[i].content != 'string')
      throw 'User and content of each comment has to be a string'
    cmm[i].user = cmm[i].user.trim()
    cmm[i].content = cmm[i].content.trim()
    if (cmm[i].user.length < 2 || cmm[i].user.length > 50)
      throw 'User of each comment has to be between 2 and 50 characters'
    if (cmm[i].content.length < 2 || cmm[i].content.length > 300)
      throw 'Content of each comment has to be between 2 and 300 characters'
  }
  return cmm
}

export function checkStatusPosts(st) {
  ...
  if (!status.includes(st))
    throw 'Invalid status provided (open, full, closed, cancelled)'
  return st
}

//TODO validate join requests fields

export function checkStatusJoin(st) {
  ...
  st = st.toLowerCase()
  if (!status.includes(st))
    throw 'Invalid status provided (pending, accepted, denied)'
  return st
}
...
export async function checkCommentId(commentId) {
  ...
  let comment = await postsCollection.findOne({
...
export function checkReportType(rt) {
  ...
  if (!reportTypes.includes(rt))
    throw 'Invalid report type provided (post, user)'
  return rt
}

export function checkStatusReport(st) {
  ...
  st = st.toLowerCase()
  if (!status.includes(st))
    throw 'Invalid status provided (open, reviewed, resolved, dismissed)'
  return st
}
